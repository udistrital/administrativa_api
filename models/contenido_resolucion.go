...
func GetTemplateResolucion(dedicacion, nivel, periodo, tipo string) (res ResolucionCompleta) {
	...
	inicio = accion
	if tipo != "1" {
		inicio = "Modificar la resolución No XXX de XXXXX de XXXX en cuanto a carga académica y valor del vínculo "
	}

	articulo = Articulo{Texto: inicio + " para el " + periodoStr + " Periodo Académico del año " + strconv.Itoa(vigencia) + ", como Docentes de " + nombreDedicacion + " (Vinculación Especial) en el escalafón y dedicación establecidos en la tabla, a los siguientes Docentes: "}
	articulos = append(articulos, articulo)
	...
	articulos = append(articulos, articulo)
	articulo = Articulo{Texto: "La presente Resolución se expide a los XXXXXXX (XX) días del mes de XXXXXX de " + strconv.Itoa(vigencia) + " y surte efectos para el " + periodoStr + " Periodo Académico del año " + strconv.Itoa(vigencia) + "."}
	articulos = append(articulos, articulo)
